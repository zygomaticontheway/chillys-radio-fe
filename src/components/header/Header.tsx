...
export default function Header() {
  // Извлекаем user из состояния state.user, поскольку данные о пользователе хранятся там
  const { user } = useAppSelector(state => state.user); // Используем state.user
  const dispatch = useAppDispatch();
    ...
    <header className={styles.header}>
      <div className={styles.topLine}>
        <Link to={homeLink.path}>
          <img src="src/media/logo.png" alt={homeLink.label} className={styles.logo} />
        </Link>
        <div className={styles.activeStation}>
          <ActivePlayedHeader />
        </div>
